...
- **Edit mode**: Single-pane only. WebSocket content refresh suppressed while editing (via `editModeRef`). `handleSave(exitAfter?)` unifies save and save-and-exit. `handleExitEdit` auto-saves if dirty (no confirm dialog). Escape key exits edit mode (auto-saves if needed). New files auto-enter edit mode via `pendingEditModeRef` in `App.tsx`. Live preview toggle (`Eye` icon) opens split view with real-time rendered preview in the secondary pane
...
- **File creation**: `POST /api/projects/:id/create-file`; `FilePlus` icon in `ExpandedProjectContent`; auto-appends `.md`; creates parent dirs; opens new file in tab and auto-enters edit mode
...
- **Toolbar menu**: `PaneToolbar.tsx` — unified `ToolbarGroup` dropdown with edit, panels, view, zoom, and autoscroll controls. Escape closes the menu (capture-phase listener with `stopPropagation`) before reaching global keyboard shortcuts. Pane container uses `onMouseDown` (not `onClick`) for `focusPane` to avoid disrupting text selection in code blocks
...
- **Keyboard shortcuts**: `Ctrl/Cmd+S` save, `+E` toggle edit, `+Shift+A` autoscroll, `+Shift+R` refresh, `+Shift+T` toggle TOC, `+K` command palette, `+W` close tab, `+[/]` switch tabs, `Esc` close toolbar menu first, then exit edit mode (auto-save) / exit fullscreen / close graph preview, `+`/`=`/`-`/`0` graph zoom
...
- **Slash commands**: Type `/` at the start of a line or after whitespace to open the slash command menu. 15 commands (headings, lists, task list, code block, quote, divider, table, image, link, bold, italic, callout). `slashCommands.ts` defines commands; `slashCommandExtension.ts` has CodeMirror state field + keymap; detection runs in React `onUpdate` callback (not ViewPlugin) to avoid dispatch-in-update issues. `SlashCommandMenu.tsx` renders floating dropdown. Arrow keys navigate, Enter/Tab selects, Escape closes
- **Copy buttons**: Code blocks and tables in `MarkdownView` have copy-to-clipboard buttons. Code blocks use `CopyButton` inline; tables use `TableCopyWrapper` with CSV-style copy
- **HR auto-format**: `hrAutoFormat()` in `packages/web/src/lib/markdownInputRules.ts` — typing `---` followed by space/enter auto-formats into a horizontal rule

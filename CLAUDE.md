...
- **Edit mode**: Single-pane only. WebSocket content refresh suppressed while editing (via `editModeRef`). `handleSave(exitAfter?)` unifies save and save-and-exit. `handleExitEdit` auto-saves if dirty (no confirm dialog). Escape key exits edit mode (auto-saves if needed). New files auto-enter edit mode via `pendingEditModeRef` in `App.tsx`
...
- **Keyboard shortcuts**: `Ctrl/Cmd+S` save, `+E` toggle edit, `+Shift+A` autoscroll, `+Shift+R` refresh, `+Shift+T` toggle TOC, `+K` command palette, `+W` close tab, `+[/]` switch tabs, `Esc` exit edit mode (auto-save) / exit fullscreen / close graph preview, `+`/`=`/`-`/`0` graph zoom
